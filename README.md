...
ブラウザでは、環境確認、プロフィール、好みの回答または全スキップ、保存同意、任意のRiot ID確認、校正、全テスト、結果、詳細診断・再テスト、任意保存、deleteTokenを使った削除、フィードバックを確認します。環境確認のREADYは実際のポインタークリックと`Q → W → E → R`入力後だけになります。準備画面は前の画面へ戻れ、質問の回答・スキップ状態を保持します。

診断中の「中断して戻る」では、未完了の測定を結果に反映しません。詳細判定・単項目再テストを中断した場合は直前の確定結果へ戻り、クイック診断を中断した場合はホームへ戻ります。ヘッダーのホーム操作やブラウザ相当の離脱操作には確認を表示します。結果を再計算したときは旧結果の保存状態を引き継がず、結果画面から改めて保存同意・保存を選びます。

結果画面では信頼度の低い能力から直接再テストでき、推薦カードからチャンピオン考察と次に試す練習を確認できます。保存・削除・フィードバックは処理中の連打を抑止し、通信失敗時は入力や結果を画面に残して再試行できます。モーダルはEscape、初期フォーカス、呼び出し元へのフォーカス復帰に対応します。

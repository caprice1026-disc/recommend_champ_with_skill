...
function ShareCard({ result, profile, cardRef }: { result: DiagnosticResult; profile: UserProfile; cardRef: React.RefObject<HTMLDivElement | null> }) {
  ...
  return <div className="share-card" ref={cardRef}><div className="share-card__top"><span>LoLスキルラボ</span><span>APTITUDE REPORT / {result.mode?.toUpperCase()}</span></div><div className="share-card__hero"><p>{profile.displayName || 'GUEST'} のプレイ適性</p><h2>{primary ? `${primary.championName} が今のあなたに近い` : 'あなたのプレイ傾向が見えました'}</h2><div className="share-card__score"><strong>{percent(result.abilities.reaction)}</strong><span>総合スナップショット<br />反応・精度・判断の複合</span></div></div><div className="share-card__bars">{Object.entries(result.abilities).slice(0, 6).map(([key, value]) => <div key={key}><span>{ABILITY_LABELS[key]}</span><i><b style={{ width: percent(value) }} /></i></div>)}</div><div className="share-card__foot"><span>lol-skill-lab / 1.0.0</span><span>診断は参考情報です</span></div></div>;
}

...
function DecisionRound({ round, detailed, onDone }: RoundProps) {
  ...
  return <div className="decision-panel"><p className="decision-panel__timer">状況 {question + 1} / {total}</p><h3>失敗できる回数が少ない状3況で、次の行動を選んでください。</h3><div className="decision-options">{options.map((option) => <button className="decision-option" type="button" key={option.title} onClick={() => choose(option.correct)}><strong>{option.title}</strong><span>{option.detail}</span></button>)}</div></div>;
}
...
export function TestStage({ testId, mode, onComplete, onAbort }: TestStageProps) {
  ...
  if (intro) return <section className="screen screen--test-intro"><TestHeader testId={testId} round="説明" onAbort={onAbort} /><div className="test-explainer"><div className="test-explainer__number">{definition.group}</div><h3>{definition.description}</h3><p>{definition.operation}</p>{testId === 'mentalStability' && <div className="notice notice--amber">このテストでは軽いプレッシャー演出が入ります。いつでも中断できます。終了後に演出の目的を説明します。</div>}<button className="button button--primary" type="button" onClick={() => setIntro(false)}>練習を始める <span>→</span></button></div></section>;

...
function TestHeader({ testId, round, onAbort }: { testId: TestId; round: string; onAbort: () => void }) {
  ...
  return (
    <div className="test-stage__header">
      <div>
        <p className="eyebrow">{definition.group} / {round === 'practice' ? '練習' : '本番'}</p>
...
type Action = 'click' | 'rightClick' | 'q' | 'e';
function CompoundInputRound({ round, detailed, onDone }: RoundProps) {
  ...
  const sequences = useMemo<Action[][]>(() => Array.from({ length: total }, (_, index) => index % 3 === 0 ? ['click', 'q', 'rightClick'] : index % 3 === 1 ? ['click', 'e', 'click', 'q'] : ['click', 'q', 'click', 'e', 'click']), [total]);
  ...
  const accept = (action: Action) => {
    if (action === current[actionIndex]) {
      if (actionIndex + 1 >= current.length) {
        ...
        setCompleted(next);
        if (sequenceIndex + 1 >= total) onDone(clamp((next * 1.1 - errors * 0.08) / total), clamp((next * current.length) / (sequences.reduce((sum, item) => sum + item.length, 0))), next, total, next + errors);
...
function PredictionRound({ round, detailed, onDone }: RoundProps) {
  ...
  return (
    <div ref={zoneRef} className="measurement-zone measurement-zone--prediction" onPointerDown={handlePointer}>
...
function AttentionRound({ round, detailed, onDone }: RoundProps) {
  ...
  useEffect(() => {
    ...
    const eventTimer = window.setInterval(() => { setEventVisible(true); setEventCount((value) => value + 1); window.setTimeout(() => setEventVisible(false), 650); }, 1600);
    const frame = (time: number) => { center.current = { x: 50 + Math.sin(time / 700) * 18, y: 50 + Math.cos(time / 900) * 16 }; if (seconds > 0) requestAnimationFrame(frame); };
  ...
  const pointerMove = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    const x = ((event.clientX - rect.left) / rect.width) * 100;
    const y = ((event.clientY - rect.top) / rect.height) * 100;
    const distance = Math.sqrt((x - center.current.x) ** 2 + (y - center.current.y) ** 2);
  ...
  return (
    <div ref={zoneRef} className="measurement-zone measurement-zone--attention" onPointerMove={pointerMove}>
      <div className="tracking-orb" style={{ left: `${center.current.x}%`, top: `${center.current.y}%` }} />
...
function TaskSwitchRound({ round, detailed, onDone }: RoundProps) {
  ...
  const [target, setTarget] = useState({ color: 'teal', action: 'click' as Action });
  const ruleChanged = trial >= Math.floor(total / 2);
  useEffect(() => { const actions: Action[] = ruleChanged ? ['q', 'click', 'e'] : ['click', 'q', 'rightClick']; const index = Math.floor(Math.random() * actions.length); setTarget({ color: ['teal', 'amber', 'violet'][index], action: actions[index] }); }, [trial, ruleChanged]);
  ...
  return <div className="measurement-zone measurement-zone--switch" onPointerDown={(event) => event.button === 0 && respond('click')} onContextMenu={(event) => { event.preventDefault(); respond('rightClick'); }}><div className={`switch-target switch-target--${target.color}`} /><p>{trial + 1} / {total}　{ruleChanged ? 'ルール変更後' : '基本ルール'}　青緑=クリック / 黄=Q / 紫=右クリック</p></div>;
}
...
function DecisionRound({ round, detailed, onDone }: RoundProps) {
  ...
  const options = useMemo(() => [
    { title: '安全に確実な選択', detail: '成功率は高いが、得られる影響は小さい', correct: true },
    { title: '大きく賭ける選択', detail: '成功時の影響は大きいが、失敗の余地が少ない', correct: false },
    { title: '情報を待つ', detail: '時間を使い、相手の動きを確認する', correct: question % 3 === 2 },
  ], [question]);
  ...
  return <div className="decision-panel"><p className="decision-panel__timer">状況 {question + 1} / {total}</p><h3>失敗できる回数が少ない状況で、次の行動を選んでください。</h3><div className="decision-options">{options.map((option) => <button className="decision-option" type="button" key={option.title} onClick={() => choose(option.correct)}><strong>{option.title}</strong><span>{option.detail}</span></button>)}</div></div>;
}
...
export function TestStage({ testId, mode, onComplete, onAbort }: TestStageProps) {
  ...
  const handleDone = (score: number, quality: number, validTrials: number, totalTrials: number, rawTrialCount = totalTrials) => {
    if (round === 'practice' && mode !== 'retest') {
      setRound('main');
      setRoundReady(false);
      window.setTimeout(() => setRoundReady(true), 100);
      return;
    }
  ...
  if (intro) return <section className="screen screen--test-intro"><TestHeader testId={testId} round="説明" onAbort={onAbort} /><div className="test-explainer"><div className="test-explainer__number">{definition.group}</div><h3>{definition.description}</h3><p>{definition.operation}</p>{testId === 'mentalStability' && <div className="notice notice--amber">このテストでは軽いプレッシャー演出が入ります。いつでも中断できます。終了後に演出の目的を説明します。</div>}<button className="button button--primary" type="button" onClick={() => setIntro(false)}>{mode === 'retest' ? '再テストを始める' : '練習を始める'} <span>→</span></button></div></section>;

...
- [ ] コミットして `main` へプッシュし、リモートと作業ツリーを確認する。
...
実装済みの候補は既存14レーンへ新規12レーンを加えた26レーン、基礎候補は24件である。結果画面には主称号を1件だけ表示し、候補カードはWiki画像を優先して表示する。バックエンド7件、フロント23件、lint、production build、ブラウザでの診断完走と画像読込を確認した。残りはコミットと `main` へのプッシュ、およびリモートSHAの照合である。
